...
/**
 * 处理与ManagerRecordEntity相关的请求的Controller。
 * 继承自BaseController类并实现IManagerRecordService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
...
@RequestMapping("/api/infra/base/identity/manager_record")
public class ManagerRecordController extends BaseController<ManagerRecordEntity, IManagerRecordService> {

    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(ManagerRecordController.class);

    @Autowired
    private IManagerRecordService service;

    /**
     * 获取ManagerRecordEntity的DataTables数据。
     *
    ...
    @Override
    public IManagerRecordService getFeign() {
        return this.service;
    }
}
