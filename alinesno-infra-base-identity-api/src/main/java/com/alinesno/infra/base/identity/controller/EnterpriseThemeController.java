...
/**
 * 处理与EnterpriseThemeEntity相关的请求的Controller。
 * 继承自BaseController类并实现IEnterpriseThemeService接口。
 *
 * @version 1.0.0
 * @author luoxiaodong
 */
@Api(tags = "EnterpriseTheme")
@RestController
@Scope(SpringInstanceScope.PROTOTYPE)
@RequestMapping("/api/infra/base/identity/enterprise_theme")
public class EnterpriseThemeController extends BaseController<EnterpriseThemeEntity, IEnterpriseThemeService> {

    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(EnterpriseThemeController.class);

    @Autowired
    private IEnterpriseThemeService service;

    /**
     * 获取EnterpriseThemeEntity的DataTables数据。
     *
