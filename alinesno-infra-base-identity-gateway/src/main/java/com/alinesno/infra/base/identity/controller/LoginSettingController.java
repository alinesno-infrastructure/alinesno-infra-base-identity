...
/**
 * 处理与LoginSettingEntity相关的请求的Controller。
...
@RequestMapping("/api/infra/base/identity/loginSetting")
public class LoginSettingController extends BaseController<LoginSettingEntity, ILoginSettingService> {

    // 日志记录
    private static final Logger log = LoggerFactory.getLogger(LoginSettingController.class);

    @Autowired
    private ILoginSettingService service;

    /**
     * 获取LoginSettingEntity的DataTables数据。
     *
    ...
    @Override
    public ILoginSettingService getFeign() {
        return this.service;
    }
}
