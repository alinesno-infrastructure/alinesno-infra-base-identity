         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="AccountList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.AccountId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:Account:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.AccountId !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:Account:remove']"></el-button>
                     </el-tooltip>
                  </template>
...
import {
   ...
   updateAccount,
   addAccount
} from "@/api/base/identity/account";
...
function getList() {
   loading.value = true;
   listAccount(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
...
function handleDelete(row) {
   const AccountIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + AccountIds + '"的数据项？').then(function () {
...
function handleUpdate(row) {
   reset();
   const AccountId = row.id || ids.value;
